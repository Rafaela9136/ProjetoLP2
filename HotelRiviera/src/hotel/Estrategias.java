...
/**
 * Essas estrategias possuem uma porcentagem que será adicionada no preco do
 * contrato. A estrategia escolhida para determinado contrato depende das suas
...
public enum Estrategias implements Serializable {
	SAO_JOAO_PREMIUM("Sao Joao Premium +50%", 23, 29, Calendar.JUNE,
			Calendar.JUNE, 1.5, Calendar.getInstance().get(Calendar.YEAR)), SAO_JOAO(
			"Sao Joao +10%", 23, 29, Calendar.JUNE, Calendar.JUNE, 1.1,
			Calendar.getInstance().get(Calendar.YEAR)), NATAL_REVEILLON(
			"Natal/Reveillon 20%", 15, 5, Calendar.DECEMBER, Calendar.JANUARY,
			1.2, Calendar.getInstance().get(Calendar.YEAR) + 1), AGOSTO(
			"Mes Agosto -20%", 1, 31, Calendar.AUGUST, Calendar.AUGUST, 0.8,
	...
	private Estrategias(String nomeEstrategia, int diaInicio, int diaFim,
		...
		this.dataInicial = new GregorianCalendar(Calendar.getInstance().get(
				Calendar.YEAR), mesInicio, diaInicio);
	...
	public String toString() {
		...
		return nomeEstrategia
				...
				+ new SimpleDateFormat("dd/MM/yyyy").format(dataInicial
						.getTime())
				+ FIM_LINHA
				+ "Data Final Estrategia: "
				+ new SimpleDateFormat("dd/MM/yyyy")
						.format(dataFinal.getTime());
	}// toString
